...
namespace AI.ZombieStateMachine
{
    public class AttackTargetState : IState
    {
        ...
        public void OnEnter()
        {
            ...
            _zombieController.zombieRigidbody.constraints = RigidbodyConstraints.FreezeRotation;
        }

        public void OnExit()
        {
            _zombieController.animator.SetBool("attackTarget", false);
            _zombieController.zombieRigidbody.constraints = RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationZ;
        }

        public void Tick()
        {
            if (Vector3.Distance(_zombieController.target.transform.position, _zombieController.transform.position) >
                _zombieController.minDistanceToTarget)
            {
